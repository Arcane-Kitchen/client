...
const MiniCalender: React.FC<MiniCalenderProps> = ({
  droppedRecipes,
  mealType,
  setMealType,
  breakfastMealPlan,
  ...
  return (
    <div
      className={`bg-[url('/sign-up-box.svg')] bg-cover bg-top w-full px-20 flex flex-col gap-2 items-center relative ${
        isOpen ? "aspect-4/1 py-5" : "aspect-15/1"
      }`}
    >
      <button
        className={`h-1/5 w-1/8 cursor-pointer text-blue-950 ${
          !isOpen && "flex-1"
        }`}
      ...
      {message && (
        <div className="absolute top-2/3 left-1/2 -translate-x-1/2 -translate-y-2/3 z-10 rounded-sm px-5 py-2 bg-black opacity-70 ">
